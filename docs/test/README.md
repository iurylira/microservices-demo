...
> `docker-compose.yml` (unit 001) brings the whole app up with Docker alone. In the
...
## Entry point
See **Prerequisites** — the entry point is `docker compose up --build` → `http://localhost:8080`
...
the frontend flows or via direct gRPC calls (`grpcurl`) from a container attached to the Compose
network (only the frontend port is published on the host).

### frontend
- **Entry point:** `http://localhost:8080` directly.
...
- Containers exiting or builds getting killed → Docker lacks resources; give it more memory/disk
  (see Prerequisites).
- Frontend 500s referencing a downstream service → that gRPC dependency isn't ready; check
  `docker compose ps` and wait for every service to be running.
